...
#include "cmaes_interface.hpp" /* <time.h> via cmaes.h */
                ...
                const char *input_parameter_filename) 
{
  ...
  strcpy(t->version, "3.11.00.beta" );

  readpara_init (&t->sp, dimension, inseed, inxstart, inrgstddev, 
                   lambda, input_parameter_filename);
  t->sp.seed = (unsigned int)random_init( &t->rand, (long unsigned int) t->sp.seed);
...
cmaes_SamplePopulation(cmaes_t *t)
{
  ...
  if(t->state == 3 || t->gen == 0)
    ++t->gen;
  t->state = 1; 
	for (iNk = 0; iNk < t->sp.lambda; ++iNk)for (i = 0; i < N; ++i)t->rgrgx[iNk][i]=((int)(t->rgrgx[iNk][i]*10000))/10000.0;
  return(t->rgrgx);
} /* SamplePopulation() */
...
for (j = 0; j < t->sp.lambda; ++j)for (i = 0; i < N; ++i)t->rgrgx[j][i]=((int)(t->rgrgx[j][i]*10000))/10000.0;	
  return(t->rgrgx);
}
...
random_init( random_t *t, long unsigned inseed)
{
  ...
  if (inseed < 1) {
    ...
    inseed = (long unsigned)fabs(100.*time(NULL)+clock());
  }
...
readpara_SupplementDefaults(readpara_t *t)
{
  ...
  if (t->seed < 1) {
    while ((int) (cloc - clock()) == 0)
      ; /* TODO: remove this for time critical applications!? */
    t->seed = (unsigned int)fabs(100.*time(NULL)+clock());
  }
